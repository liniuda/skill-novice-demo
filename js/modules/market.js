...
import { buildSceneRecommendBanner } from './engines.js';
...
const _rendered  = { square: false, publish: false, mine: false, knowledge: false };

/* 社区知识包（Market 版）——面向跨空间共享的精选知识包 */
const KB_PACKS = [
  {
    id: 'kp-refund-core',
    icon: '💰',
    name: '退款业务核心知识包',
    desc: '涵盖 BLIK · Alibaba.com Pay · 外贸服务市场 · 生意助手 等 5 大渠道退款规则',
    categoryId: '87875007',
    entries: 82,
    downloads: 1840,
    stars: 4.9,
  ...
  {
    id: 'kp-logistics-full',
    icon: '📦',
    name: '物流全链路知识包',
    desc: '商家退款·买家物流市场·物流跟踪·清关异常·赔付标准全覆盖',
    categoryId: '4724',
    entries: 128,
    downloads: 2310,
    stars: 4.8,
  ...
  },
  {
    id: 'kp-taplus-gs',
    icon: '🌏',
    name: '港台 GS / TA Plus 业务包',
    desc: '信保服务·拒付综合服务·TA Plus 入驻与退出·护航赔付',
    categoryId: '507140007',
    entries: 47,
    downloads: 678,
    stars: 4.7,
    author: 'alibaba-cco',
  ...
  {
    id: 'kp-epr-compliance',
    icon: '📜',
    name: 'EPR 合规与跨境注册包',
    desc: '欧盟 EPR 注册材料·税号申报·产品分类·平台合规护航',
    categoryId: '74774027',
    entries: 96,
    downloads: 1520,
    stars: 4.8,
    author: 'cco-compliance-team',
    badge: '热门',
    badgeType: 'hot',
    avgScore: 0.75,
    tone: 'orange-red',
  ...
  {
    id: 'kp-ai-copilot-onboard',
    icon: '🤖',
    name: 'AI Copilot 接入手册',
    desc: '后台开通·Prompt 模板配置·灰度发布·评测与优化闭环',
    categoryId: '276038076',
    entries: 54,
    downloads: 420,
    stars: 4.6,
    author: 'cco-product-team',
  ...
  {
    id: 'kp-dispute-evidence',
    icon: '⚖️',
    name: '纠纷举证知识包',
    desc: '举证材料清单·上传路径·时效要求·【驳如果无】应对话术',
    categoryId: '276038014',
    entries: 73,
    downloads: 932,
    stars: 4.7,
    author: 'cco-dispute-team',
...
/* fork 过的知识包 —— localStorage */
function getForkedPacks() {
  try {
    const raw = localStorage.getItem('scd_market_forked_packs');
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}
function saveForkedPacks(list) {
  try { localStorage.setItem('scd_market_forked_packs', JSON.stringify(list)); } catch {}
}
...
export function renderMarketSub(tab) {
  ...
  if (!_rendered[tab]) {
    ...
    if (tab === 'knowledge') { view.innerHTML = renderKnowledgeMarket(); bindKnowledgeMarketEvents(view); }
...
───────────────────────────────────────────── */
/* ───────────────────────────────────
   社区知识广场（knowledge）—— Market 版
─────────────────────────────────── */
function renderKnowledgeMarket() {
  const forked = new Set(getForkedPacks());
  return `
    <div class="page-header">
      <h1>📚 知识广场</h1>
      <p>跨空间共享的社区知识包 · 一键 fork 到当前产品空间 · 接入 RAG 兜底链路</p>
    </div>

    <div class="kbm-hero">
      <div class="kbm-hero-text">
        <div class="kbm-hero-title">只需一次 fork，让你的 Skill 立刻拥有行业级兜底</div>
        <div class="kbm-hero-sub">社区精选 ${KB_PACKS.length} 个知识包 · 累计 ${KB_PACKS.reduce((s,p)=>s+p.downloads,0).toLocaleString()} 次被引用 · 平均 score 0.74</div>
      </div>
      <div class="kbm-hero-stat">
        <div class="kbm-hero-stat-num">${forked.size}</div>
        <div class="kbm-hero-stat-label">已 fork</div>
      </div>
    </div>

    <div class="kbm-grid">
      ${KB_PACKS.map(p => {
        const isForked = forked.has(p.id);
        const badgeHtml = p.badge ? `<span class="kbm-badge kbm-badge-${p.badgeType}">${p.badge}</span>` : '';
        return `
          <div class="kbm-card" data-id="${p.id}">
            <div class="kbm-card-head">
              <span class="kbm-card-icon" data-tone="${p.tone}">${p.icon}</span>
              <div class="kbm-card-title-wrap">
                <div class="kbm-card-title">${escHtml(p.name)}${badgeHtml}</div>
                <div class="kbm-card-author">@${p.author}</div>
              </div>
            </div>
            <div class="kbm-card-desc">${escHtml(p.desc)}</div>
            <div class="kbm-card-meta">
              <span>📄 ${p.entries} 条</span>
              <span>⬇ ${p.downloads.toLocaleString()}</span>
              <span>★ ${p.stars}</span>
              <span class="kbm-card-score">avg ${p.avgScore.toFixed(2)}</span>
            </div>
            <div class="kbm-card-foot">
              <button class="btn btn-ghost btn-sm" data-act="preview">预览目录</button>
              <button class="btn ${isForked ? 'btn-ghost' : 'btn-primary'} btn-sm" data-act="fork" ${isForked ? 'disabled' : ''}>
                ${isForked ? '✓ 已 fork' : '一键 fork'}
              </button>
            </div>
          </div>
        `;
...
function bindKnowledgeMarketEvents(view) {
  view.querySelectorAll('.kbm-card').forEach(card => {
    const id = card.dataset.id;
    const pack = KB_PACKS.find(p => p.id === id);
    if (!pack) return;
    card.querySelector('[data-act="preview"]')?.addEventListener('click', () => {
      showToast(`📖 预览「${pack.name}」· ${pack.entries} 条知识`, 'info');
    });
    card.querySelector('[data-act="fork"]')?.addEventListener('click', () => {
      const list = getForkedPacks();
      if (list.includes(id)) return;
      list.push(id);
      saveForkedPacks(list);
      showToast(`✅ 已 fork「${pack.name}」到当前空间，可在 Work 知识库查看`);
      refreshSub('knowledge');
    });
  });
}
    ...
    return `
      <div class="mine-card${hasUpdate ? ' mine-card-update' : ''}${workGone ? ' mine-card-gone' : ''}">
        ...
        <div class="mine-card-actions">
          <button class="btn btn-sm btn-ghost" data-engine="attribution" data-record-id="${escHtml(record.id)}">📈 效果归因</button>
